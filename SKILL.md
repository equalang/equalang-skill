...
```bash
# What would it cost? Uploads without starting anything; free. Keeps the file for an hour.
...
- **Language codes** look like `en`, `zh-CN`, `zh-TW`, `ja`, `pt`; `languages <name>` finds one. Omit `--source-language` to have it detected.
