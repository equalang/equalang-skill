...
> 请按 https://equalang.com/install/skill-install.md 的说明，安装 Equalang skill。
...
每条命令都输出一段 JSON：成功时告诉你文件写到了哪、花了多少积分，失败时告诉你哪里出了问题。任何命令加上 `--help` 就能看到它的参数。智能体自己读的是 [SKILL.md](../SKILL.md)。

语言代码形如 `en`、`zh-CN`、`ja`。跑 `languages` 看全部，或者 `languages chinese` 搜索。任务要跑几分钟——命令会一直等着，中断了也不要紧，用 `status <job_id>` 可以重新接上。
